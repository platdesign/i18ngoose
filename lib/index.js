...
module.exports = (schema, options) => {
	...
	schema.eachPath((path, config) => {
		transformSchema(path, config, schema);
	});


	function transformSchema(path, config, schema) {
		...
		if (config.options.i18n === true) {
			...
			if(config.instance === 'Array' && config.schema) {

				config.schema.eachPath((_path, _config) => {
					transformSchema(_path, _config, config.schema);
				});
			}
		}
